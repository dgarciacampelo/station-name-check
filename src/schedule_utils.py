...
def quarterly_database_backup():
    "Performs a database backup, only on the first day of each quarter."
    if datetime.now().day != 1 or datetime.now().month % 3 != 1:
        return
...
async def setup_schedules():
    ...
    schedule.every().day.at("21:00", "Europe/Madrid").do(quarterly_database_backup)
    schedule.every().day.at("21:30", "Europe/Madrid").do(conditional_database_backup)
